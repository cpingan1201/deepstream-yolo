...
UPLOAD_CONFIG = {
    ...
    "app_id": "3959800e-1e7b-11f1-8e32-246e96b0c972",
    "app_secret": "39598042-1e7b-11f1-8e32-246e96b0c972",
...
WATCHDOG_SOURCE_TIMEOUT = float(os.getenv("WATCHDOG_SOURCE_TIMEOUT", "90"))
...
REVIVE_WORKER_COUNT = int(os.getenv("REVIVE_WORKER_COUNT", "4"))
SOURCE_INITIAL_CONNECT_TIMEOUT = float(os.getenv("SOURCE_INITIAL_CONNECT_TIMEOUT", "120"))
SOURCE_START_STAGGER = float(os.getenv("SOURCE_START_STAGGER", "0.2"))
WATCHDOG_REVIVE_BATCH_LIMIT = int(os.getenv("WATCHDOG_REVIVE_BATCH_LIMIT", "20"))
...
VEHICLE_VIOLATION_DURATION_THRESHOLD = float(os.getenv("VEHICLE_VIOLATION_DURATION_THRESHOLD", "10.0"))
...
class FireSmokeDetector:
    ...
    def process(self, label, cam, conf):
        ...
        print(f"[{label.upper()}] {cam}: 检测持续 {duration:.1f} 秒，触发告警")
        return True
...
class VehicleTracker:
    ...
    def update(self, bbox, now, is_ev_mode=False, in_zone=True):
        """更新车辆位置"""
        cx = (bbox[0] + bbox[2]) / 2
        cy = (bbox[1] + bbox[3]) / 2
        ...
        if self.last_position:
            ...
            if distance < self.stationary_threshold:
                self.stationary_time += (now - self.last_update_time)
            else:
                # 车辆移动，重置计时
                self.stationary_time = 0
                self.start_time = now
        ...
        # 检查是否违停
        if self.stationary_time >= self.violation_time_threshold and not self.violation_recorded:
            self.violation_recorded = True
            return True  # 违停

        return False

    def is_timeout(self, timeout=300):
        """检查是否超时清理"""
    ...
    def send_notification(self, camera_name, detection_type, file_path="", vlm_result=""):
        ...
        try:
            ...
            for cam_key, cam_config in self.camera_configs.items():
                if cam_config.get("display_name") == camera_name:
                    camera_ip = cam_config.get("rtsp_url", "").split("@")[1].split(":")[0] if "@" in cam_config.get("rtsp_url", "") else ""
                    break
...
class AsyncElevatorDetector:
    ...
    def enqueue_ocr(self, frame_bgr, camera_name, person_count):
        """将帧放入OCR检测队列"""
        try:
            self.ocr_queue.put_nowait((frame_bgr, camera_name, person_count))
    ...
    def enqueue_obj(self, frame_bgr, camera_name):
        """将帧放入YOLO-World检测队列"""
        try:
            self.obj_queue.put_nowait((frame_bgr, camera_name))
            self.update_queue_metrics()
            return True
        except queue.Full:
    ...
    def _ocr_worker(self):
        ...
        while self.running:
            try:
                frame_bgr, camera_name, person_count = self.ocr_queue.get(timeout=1.0)
                self.update_queue_metrics()
    ...
    def _obj_worker(self):
        ...
        while self.running:
            try:
                frame_bgr, camera_name = self.obj_queue.get(timeout=1.0)
                self.update_queue_metrics()
    ...
    def _get_elevator_detection_zone(self, camera_name):
        """获取电梯检测区域 - 支持通过display_name或camera_key查找"""
        # 首先尝试直接匹配camera_key
        ...
        for cam_key, cam_cfg in self.camera_configs.items():
            ...
            if display_name == camera_name.lower():
                ...
                if zone:
                    ...
                    return zone
            # 也检查name字段
            name = cam_cfg.get('name', '').lower()
            if name == camera_name.lower():
                zone = cam_cfg.get('elevator_detection_zone')
                if zone:
                    if 'points' in zone:
                        pts = zone['points']
...
class ImageSaver:
    ...
    def enqueue(self, frame_bgr, camera_name, alarm_type, bbox_list=None):
        """bbox_list: [(label, confidence, (x1,y1,x2,y2)), ...]"""
        try:
            self.queue.put_nowait((frame_bgr, camera_name, alarm_type, bbox_list))
            self.update_queue_metrics()
        except queue.Full:
            if METRICS_ENABLED:
    ...
    def _worker(self):
        while self.running:
            try:
                frame, cam_name, a_type, bbox_list = self.queue.get(timeout=1)
                self.update_queue_metrics()
            except queue.Empty:
                continue
            try:
...
class DetectionHandler:
    ...
    def __init__(self, config=None, camera_configs=None):
        ...
        # 违停跟踪器: {tracker_key: {'tracker': VehicleTracker, 'last_bbox': bbox, 'start_time': time}}
        ...
        if camera_configs:
            for cam_key, cam_config in camera_configs.items():
                ...
                cam_name = cam_config.get('display_name') or cam_config.get('name', cam_key)
    ...
    def process_detection(self, camera_id, camera_name, class_id, confidence, bbox=None, frame_width=640, frame_height=480, tracker_id=None):
        """处理检测结果"""
        ...
        if class_id in [1, 2, 3]:
            vehicle_violation_type = self._check_vehicle_violation(camera_name, class_id, bbox, confidence, tracker_id)
        ...
        if class_id in [4, 5]:
            fire_smoke_triggered = self._check_fire_smoke(camera_name, label, confidence)

        # 宠物检测 - 无需追踪
        if class_id == 6:
            self._check_pet(camera_name)

        # 摔倒检测 - person(0)，使用tracker_id追踪
        fall_triggered = False
        if class_id == 0:
            result = self._check_fall(camera_name, bbox, confidence, tracker_id)
            if result:
                fall_triggered = True
        ...
        if class_id == 0:
            result = self._check_fight(camera_name, bbox, confidence, tracker_id)
            if result:
    ...
    def _check_vehicle_violation(self, camera_name, class_id, bbox, confidence, tracker_id=None):
        """检查车辆违停 - 同一 tracker_id 持续 10 秒视为违停，同一 ID 只告警一次"""
        if bbox is None:
            return None

        # 使用tracker_id作为唯一标识，如果没有则用class_id
        if tracker_id is not None:
            key_prefix = f"{camera_name}_{tracker_id}"
        else:
            key_prefix = f"{camera_name}_{class_id}"
    ...
    def _check_fire_smoke(self, camera_name, label, confidence):
        """火焰/烟雾检测"""
        current_time = time.time()
        ...
        triggered = self.fire_smoke_detector.process(label, camera_name, confidence)
        if triggered:
            self.stats[label] = self.stats.get(label, 0) + 1
        return triggered

    def _check_pet(self, camera_name):
        """宠物检测"""
        print(f"[宠物] {camera_name}: 检测到宠物")

    def _check_person_bound_alert(self, camera_name, alert_type, person_tracker_id, label=None):
        """同一 camera + person tracker + alert_type 只告警一次，超时后清理状态。"""
        if person_tracker_id is None or person_tracker_id == 18446744073709551615:
            return False

        current_time = time.time()
        key = f"{camera_name}_{person_tracker_id}_{alert_type}"
    ...
    def _check_fall(self, camera_name, bbox, confidence, tracker_id=None):
        ...
        # 使用tracker_id作为唯一标识
        if tracker_id is not None:
            person_key = f"{camera_name}_{tracker_id}"
        else:
            return

        with self.lock:
            if camera_name not in self.person_trackers:
                self.person_trackers[camera_name] = {
                    'persons': {},
                    'frame_count': 0
                }

            if person_key not in self.person_trackers[camera_name]['persons']:
                # 新人
                self.person_trackers[camera_name]['persons'][person_key] = {
                    'bbox': bbox,
                    'width': w,
                    'height': h,
                ...
                # 更新已有的人
                p = self.person_trackers[camera_name]['persons'][person_key]
            ...
            # 清理超时的人（30秒没更新）
            self.person_trackers[camera_name]['frame_count'] += 1
            timeout_pids = []
            for pid, pdata in self.person_trackers[camera_name]['persons'].items():
                if current_time - pdata.get('last_time', 0) > 30:
                    timeout_pids.append(pid)
            for pid in timeout_pids:
                del self.person_trackers[camera_name]['persons'][pid]

    def _check_fight(self, camera_name, bbox, confidence, tracker_id=None):
        """
        ...
        with self.lock:
            if camera_name not in self.fight_trackers:
                self.fight_trackers[camera_name] = {
                    'persons': {},
                    'pairs': {},
                    'last_alert_time': 0
                }

            ft = self.fight_trackers[camera_name]
...
class RealTimeUploader:
    def __init__(self, camera_configs=None):
        ...
        self.app_id = "3959800e-1e7b-11f1-8e32-246e96b0c972"
        self.app_secret = "39598042-1e7b-11f1-8e32-246e96b0c972"
        self.auth_header = {
    ...
    def _send_notification(self, camera_name, detection_type, file_path, camera_ip="", vlm_json_path="", vlm_result_data=None):
        try:
...
class FrameProcessor:
    ...
    def __init__(self, detection_handler, image_saver, vlm_manager, uploader, async_elevator_detector, class_thresholds, camera_id_to_name, gpu_id, pipeline_id, frame_stats, camera_process_last_seen, camera_process_ever_seen, process_seen_lock):
            ...
            try:
                ...
                try:
                        ...
                        if class_id in [7, 8]:
                            ...
                            if best_person_tid is not None:
                                if class_id == 8 and self.detection_handler._check_person_bound_alert(
                                    camera_name, "dangerous_item", best_person_tid, label=label
                                ):
                                    ...
                                    camera_name, "cigarette", best_person_tid, label=label
                                ):
                                    should_save_frame, alarm_label, bbox_list = set_alarm(
                    ...
                    if should_save_frame:
                        try:
                            self.image_saver.enqueue(get_frame_copy(), camera_name, alarm_label, bbox_list)
                        except Exception as e:
                    ...
                    if self.async_elevator_detector and camera_name in self.detection_handler.elevator_status.camera_status:
                        ...
                        if need_elevator_ocr:
                            if frame_is_fresh:
                                try:
                                    self.async_elevator_detector.enqueue_ocr(get_frame_copy(), camera_name, person_count)
                                except Exception as e:
                        ...
                        if need_elevator_obj:
                            if frame_is_fresh:
                                try:
                                    if self.async_elevator_detector.enqueue_obj(get_frame_copy(), camera_name):
...
class DeepStreamApp:
    def __init__(self, cameras_data, camera_configs=None, detection_handler=None, base_dir=None, gpu_id=0, pipeline_id=0, global_thresholds=None):
        ...
        self.cameras_data = cameras_data  # List of dicts with url, name, encoding
        self.camera_configs = camera_configs or {}
    ...
    def _create_elements(self):
        ...
        for i, cam in enumerate(self.cameras_data):
            ...
            encoding = cam.get("encoding", "h265")  # 默认 H265
            ...
            # 根据URL类型选择不同的源处理方式
            if url.endswith(".flv") or url.endswith(".m3u8") or url.startswith("http://") or url.startswith("https://") or url.startswith("rtsp://"):
                ...
                def on_source_setup(decodebin, source):
                    try:
                        try:
                            source.set_property("name", f"rtspsrc_{i}")
                        except Exception:
                            pass
                ...
                rtspsrc.set_property("protocols", 4)
                rtspsrc.set_property("latency", 500)
                rtspsrc.set_property("tcp-timeout", 5000000)  # 5秒
                rtspsrc.set_property("timeout", 5000000)  # 5秒
                rtspsrc.set_property("drop-on-latency", 1)  # 关键：丢弃过时包

                # ✅ 方案A：添加 video-only capsfilter bin，过滤掉音频pad
                # 这样所有音频pad会被capsfilter直接拒绝，不会流到后续元素
                video_filter_bin = Gst.Bin.new(f"video_filter_{i}")
                video_filter_queue = Gst.ElementFactory.make("queue", f"video_filter_queue_{i}")
                video_filter_caps = Gst.ElementFactory.make("capsfilter", f"video_filter_caps_{i}")
                # ✅ 只允许 video 开头的 caps，audio/* 全部被这只 caps 丢弃
                video_only_caps = Gst.Caps.from_string("video/x-raw; video/x-h264; video/x-h265; video/mpeg; video/x-rtp; video/x-rtp-empty")
                video_filter_caps.set_property("caps", video_only_caps)
                video_filter_bin.add(video_filter_queue)
                video_filter_bin.add(video_filter_caps)
                video_filter_queue.link(video_filter_caps)
                # Ghost pad：bin 的 sink 连接到 rtspsrc
                ghost_sink = Gst.GhostPad.new("sink", video_filter_queue.get_static_pad("sink"))
                video_filter_bin.add_pad(ghost_sink)
                # Ghost pad：bin 的 src 连接 depay
                ghost_src = Gst.GhostPad.new("src", video_filter_caps.get_static_pad("src"))
                video_filter_bin.add_pad(ghost_src)
                self.pipeline.add(video_filter_bin)
                ...
                if encoding == "h264":
                    ...
                    depay = Gst.ElementFactory.make("rtph265depay", f"depay_{i}")
                    parser = Gst.ElementFactory.make("h265parse", f"parse_{i}")
                print(f"{self.log_prefix} Using {encoding.upper()} encoding for {cam_name}")
                decoder = Gst.ElementFactory.make("nvv4l2decoder", f"decoder_{i}")

                # ==============================================
                # 🛠️ 调优 2: 强力压缩 Decoder 显存占用 (防止 OOM)
                # ==============================================
                decoder.set_property("num-extra-surfaces", 1)  # 极度缩减显存
                decoder.set_property("drop-frame-interval", 0)  # 全帧输出

                # 添加到 pipeline
                rtspsrc.set_locked_state(True)  # ⚡ 锁定状态！引擎热身完毕前不准点火！
                self.pipeline.add(rtspsrc)
                self.pipeline.add(depay)
                self.pipeline.add(parser)
                self.pipeline.add(decoder)

                # 直接将 rtspsrc 链接到 video_filter_bin，再链接到 depay
                rtspsrc.link(video_filter_bin)
                # ✅ 简化后的 pad-added callback：由于音频已被过滤，只需直接 link 到 depay
                def make_rtspsrc_callback(depay_elem):
                    def callback(el, src_pad):
                        caps = src_pad.query_caps(None)
                        # 由于已经有了 video_filter_bin，这里理论上只会收到视频 pad
                        if caps and "video" in caps.to_string().lower():
                            ...
                            src_pad.link(sink_pad)
                            # print(f"[DYNAMIC LINK ✅] RTSP 已连通，成功接入 Depay！")
                    return callback
                rtspsrc.connect("pad-added", make_rtspsrc_callback(depay))

                # depay -> parser -> decoder
                depay.link(parser)
                parser.link(decoder)

                # decoder -> queue -> streammux (静态链接)
    ...
    def run(self):
        ...
        def delayed_ignition():
            ...
            for i in range(self.num_sources):
                try:
                    ...
                    if src:
                        src.set_locked_state(False)  # 解除锁定
                        src.set_state(Gst.State.PLAYING)  # 正式连接网络！
                        time.sleep(SOURCE_START_STAGGER)  # 错峰点火，防止瞬间拥爆网卡
                except Exception as e:
                    pass
            with self.probe_stats_lock:
                self.decode_stats_snapshot_time = time.time()
        ...
        # 挂载闹钟：启动 20 秒后执行批量点火
        GLib.timeout_add_seconds(20, delayed_ignition)
    ...
    def _watchdog_worker(self):
        ...
        while not self._stop_event.is_set():
            ...
            for source_id, last_seen in list(source_last_seen_map.items()):
                ...
                source_idle = max(0.0, current_time - last_seen) if last_seen else (current_time - self.pipeline_started_at)
            ...
            try:
                ...
                try:
                    ...
                    if uri:
                        ...
                        print(f"{self.log_prefix} REVIVE: {cam_name} RTSP source restarted")
                    else:
                    ...
                    while time.time() - reconnect_start < REVIVE_VALIDATE_TIMEOUT:
                        time.sleep(1)
    ...
    def _on_bus_message(self, bus, msg):
        if msg.type == Gst.MessageType.ERROR:
            ...
            if ("Not Found" in err_msg or "enough data" in err_msg) and source_id is not None:
...
if __name__ == '__main__':
    ...
    for cam_key, cam_data in camera_configs.items():
        if not cam_data.get("disabled", False):
            cameras_data.append({
                "url": cam_data["rtsp_url"],
                "name": cam_data.get("display_name", cam_key),
                "encoding": cam_data.get("encoding", "h265")
            })
